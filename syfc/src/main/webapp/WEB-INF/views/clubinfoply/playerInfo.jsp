								...
								<tbody>
									<c:forEach var="dto" items="${list}">
										<tr>
											...
											<td>
												<c:choose>
													<c:when test="${dto.position == 'FW'}"><span class="pos-badge pos-fw">FW</span></c:when>
													<c:when test="${dto.position == 'MF'}"><span class="pos-badge pos-mf">MF</span></c:when>
													<c:when test="${dto.position == 'DF'}"><span class="pos-badge pos-df">DF</span></c:when>
													<c:when test="${dto.position == 'GK'}"><span class="pos-badge pos-gk">GK</span></c:when>
													<c:otherwise><span class="pos-badge pos-etc">${dto.position}</span></c:otherwise>
												</c:choose>
											</td>
											...
											<td>
												<c:choose>
													...
													<c:when test="${dto.status == 1}"><span class="status-pill status-active">활동</span></c:when>
													<c:otherwise><span class="status-pill status-inactive">제적</span></c:otherwise>
												</c:choose>
											</td>
										</tr>
									</c:forEach>
